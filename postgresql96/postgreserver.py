...
def stop():
    ...
    return 0
def startlog():
    os.system("su - postgres -l -c \"sed -i 's/\(wal_level\ =\).*/\\1\ replica/' /var/lib/pgsql/9.6/data/postgresql.conf\"")
    ...
    os.system("su - root -l -c \"sed -i \\\"s/\\(archive_command\\ =\\).*/\\1\\ 'cp\\ %p\\ \\/opt\\/chsql\\/log\\/%f'/g\\\" /var/lib/pgsql/9.6/data/postgresql.conf\"")
...
def stoplog():
    ...
    os.system("su - postgres -l -c \"sed -i 's/max_wal_size\ =/#max_wal_size\ =/g' /var/lib/pgsql/9.6/data/postgresql.conf\"")
    os.system("su - postgres -l -c \"sed -i 's/min_wal_size\ =/#min_wal_size\ =/g' /var/lib/pgsql/9.6/data/postgresql.conf\"")
    os.system("su - postgres -l -c \"sed -i 's/\(logging_collector\ =\).*/\\1\ off/' /var/lib/pgsql/9.6/data/postgresql.conf\"")
    os.system("service postgresql-9.6 restart")
    return 0
def backup():
    os.system("service postgresql-9.6 restart")
