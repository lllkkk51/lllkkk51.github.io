...
def chpassword(x):
    ...
    os.system("mysql -uroot -e \"use mysql;update user set authentication_string=password(%s) where user='root';flush privileges;\"" % (x))
...
def stop():
    ...
    return 0
def startlog():
    os.system("source /opt/chsql/iniset.sh && iniset -sudo /etc/my.cnf mysqld log_bin /opt/chsql/log/mysqlbin.log on && iniset -sudo /etc/my.cnf mysqld server_id 1")
    os.system("service mysqld restart")
    return 0
def stoplog():
